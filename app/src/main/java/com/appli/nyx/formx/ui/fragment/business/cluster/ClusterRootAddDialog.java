...
public class ClusterRootAddDialog extends BaseDialogFragment<ClusterViewModel> {
	...
	public void save(View view) {
		...
		FirebaseFirestore.getInstance().collection(CLUSTER_PATH).document(SessionUtils.getUserUid()).collection(CLUSTER_DATA).add(cluster).addOnCompleteListener(task -> {
			if (task.isSuccessful()) {
				NavHostFragment.findNavController(ClusterRootAddDialog.this).navigate(R.id.action_global_clusterListFragment);
			} else {
